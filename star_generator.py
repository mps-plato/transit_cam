...
class Rect(pygame.Rect):
    ...
    def from_yaml(yaml_node):
        if yaml_node is None:
            return Rect()
        return Rect(yaml_node.get(YAML_LEFT, 0),
...
def get_default_star(size):
    return Rect(size[0]/2-min(size)/4, 
             size[1]/2-min(size)/4,
             min(size)/2,
             min(size)/2,
            )
...
def get_default_spot(size):
    default_star = get_default_star(size)                          
    return Rect(default_star.left+default_star.width/4,
...
class SimStatus(object):
    def __init__(self, screen_size = DEFAULT_SIZE, star = get_default_star(DEFAULT_SIZE), spot = get_default_spot(DEFAULT_SIZE)):
    ...
    @staticmethod
    def from_yaml(yaml):
        result = SimStatus(DEFAULT_SIZE, get_default_star(DEFAULT_SIZE))

        result.amplitude = yaml.get(YAML_AMPLITUDE, result.amplitude)
        result.pulsating = yaml.get(YAML_PULSATING, result.pulsating)
        result.spot_visible = yaml.get(YAML_SPOT_VISIBLE, result.spot_visible)
        result.period = yaml.get(YAML_PERIOD, result.period)
        if YAML_STAR in yaml.keys():
            result.star = Rect.from_yaml(yaml[YAML_STAR])
        if YAML_SPOT in yaml.keys():
            result.spot = Rect.from_yaml(yaml[YAML_SPOT])
    ...
    def load_state():
        ...
        with open(CONFIG_FILE, 'r') as input:
            return SimStatus.from_yaml(yaml.load(input))
    ...
    def reduce_star(self):
        print('Reducing star')
        if self.star.width > 2*SIZE_STEP and self.star.height > 2*SIZE_STEP:
            self.star = Rect(self.star.left+SIZE_STEP, self.star.top+SIZE_STEP, self.star.width-2*SIZE_STEP, self.star.height-2*SIZE_STEP)
        self.save_state()
    ...
    def widen_star(self):
        print('Widening star')
        self.star = Rect(self.star.left-SIZE_STEP, self.star.top, self.star.width+2*SIZE_STEP, self.star.height)
        self.save_state()
        
    def narrow_star(self):
        print('Narrowing star')
        if self.star.width > 2*SIZE_STEP:
            self.star = Rect(self.star.left+SIZE_STEP, self.star.top, self.star.width-2*SIZE_STEP, self.star.height)
        self.save_state()
            
    def reduce_spot(self):
        print('Reducing spot')
        if self.spot.width > 2*SIZE_STEP and self.spot.height > 2*SIZE_STEP:
            self.spot = Rect(self.spot.left+SIZE_STEP, self.spot.top+SIZE_STEP, self.spot.width-2*SIZE_STEP, self.spot.height-2*SIZE_STEP)
        self.save_state()
    ...
    def widen_spot(self):
        print('Widening spot')
        self.spot = Rect(self.spot.left-SIZE_STEP, self.spot.top, self.spot.width+2*SIZE_STEP, self.spot.height)
        self.save_state()
        
    def narrow_spot(self):
        print('Narrowing spot')
        if self.spot.width > 2*SIZE_STEP:
            self.spot = Rect(self.spot.left+SIZE_STEP, self.spot.top, self.spot.width-2*SIZE_STEP, self.spot.height)
        self.save_state()
    ...
    def move_spot_left(self):
        ...
        if self.spot.left > 2*SIZE_STEP:
            self.spot = Rect(self.spot.left-2*SIZE_STEP, self.spot.top, self.spot.width, self.spot.height)
        self.save_state()
    ...
    def move_spot_up(self):
        ...
        if self.spot.top > 2*SIZE_STEP:
            self.spot = Rect(self.spot.left, self.spot.top-2*SIZE_STEP, self.spot.width, self.spot.height)
        self.save_state()
    ...
    def reset_star(self):
        ...
        self.star = get_default_star(self.screen_size)
        self.save_state()
        
    def reset_spot(self):
        print('Resetting spot')
        self.spot = get_default_spot(self.screen_size)
        self.save_state()
        
    def make_star_round(self):
        print('Rounding star')
        self.star = Rect(self.star.left+(self.star.width-self.star.height)/2, self.star.top, self.star.height, self.star.height)
        self.save_state()

    def make_spot_round(self):
        print('Rounding spot')
        self.spot = Rect(self.spot.left+(self.spot.width-self.spot.height)/2, self.spot.top, self.spot.height, self.spot.height)
        self.save_state()
    ...
    def handle_key_event(self, key, value, pressed_down = True):
        ...
        if self.pressed_keys.get(pygame.K_ESCAPE, None) == 0:
            self.done = True
        ...
        elif self.pressed_keys.get(pygame.K_DOWN, None) in [pygame.KMOD_LCTRL | pygame.KMOD_LSHIFT, pygame.KMOD_RCTRL | pygame.KMOD_RSHIFT]:
            self.reset_star()
        ...
        elif self.pressed_keys.get(pygame.K_DOWN, None) in [pygame.KMOD_LCTRL | pygame.KMOD_LALT | pygame.KMOD_LSHIFT, pygame.KMOD_RCTRL | pygame.KMOD_RALT | pygame.KMOD_RSHIFT]:
            self.reset_spot()
            del self.pressed_keys[pygame.K_DOWN]
        ###
        # Spot position
    ...
    def sine_rect(base_rect, amplitudes, time, period):
        ...
        print (time, ratio) 
        return Rect(base_rect.left-amplitudes[0]*ratio, 
